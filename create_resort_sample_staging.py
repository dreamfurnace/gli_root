#!/usr/bin/env python3
"""
스테이징 환경에 GLI 그랜드 아주르 리조트 & 스파 샘플 데이터 생성 스크립트
"""
...
def get_category_and_product_type():
    """카테고리 및 상품 유형 찾기 (이름 기반)"""
    try:
        print("📋 카테고리 검색 중...")
        category = ShoppingCategory.objects.filter(
            name__contains="리조트"
        ).first() or ShoppingCategory.objects.filter(
            name__contains="호텔"
        ).first()

        if not category:
            print("❌ 리조트/호텔 카테고리를 찾을 수 없습니다.")
            all_cats = ShoppingCategory.objects.all()
            print(f"   사용 가능한 카테고리: {list(all_cats.values_list('name', flat=True))}")
            return None, None

        print(f"✅ 카테고리 발견: {category.name} (ID: {category.id})")

        print("📦 상품 유형 검색 중...")
        product_type = ShoppingProductType.objects.filter(
            category=category,
            name__contains="리조트"
        ).first()

        if not product_type:
            print(f"❌ 리조트 상품 유형을 찾을 수 없습니다.")
            all_types = ShoppingProductType.objects.filter(category=category)
            print(f"   사용 가능한 상품 유형: {list(all_types.values_list('name', flat=True))}")
            return None, None

        print(f"✅ 상품 유형 발견: {product_type.name} (ID: {product_type.id})")
        return category, product_type

    except Exception as e:
        print(f"❌ 카테고리/상품 유형 조회 중 오류: {e}")
        import traceback
        traceback.print_exc()
        return None, None
...
def create_resort_sample():
    ...
    # 카테고리 및 상품 유형 확인
    category, product_type = get_category_and_product_type()
    if not category or not product_type:
        return

    # 동일한 이름의 상품이 이미 존재하는지 확인
    existing = ShoppingProduct.objects.filter(name="GLI 그랜드 아주르 리조트 & 스파").first()
    if existing:
        print(f"⚠️  이미 동일한 이름의 상품이 존재합니다 (ID: {existing.id})")
        print(f"   기존 상품을 사용하거나 삭제 후 재실행하세요.")
        return existing

    # 리조트 상품 생성
    try:
        # product_type_id를 직접 지정
        resort = ShoppingProduct(
        ...
        print(f"✅ 스테이징 환경에 리조트 샘플 데이터가 생성되었습니다!")
        ...
        print(f"\n📱 스테이징 확인: https://gli-stg.click/shopping/resort/{resort.id}")
...
if __name__ == "__main__":
    print("=" * 60)
    print("GLI 그랜드 아주르 리조트 & 스파 샘플 데이터 생성 (스테이징)")
    print("=" * 60)
