...
try:
    ...
    print(f"✅ 패키지 카테고리 확인: {package_cat.id}")
except ShoppingCategory.DoesNotExist:
    print("❌ 패키지 카테고리가 존재하지 않습니다. 먼저 마이그레이션을 실행해주세요.")
    sys.exit(1)

try:
    package_type = ShoppingProductType.objects.get(code='package', category=package_cat)
    print(f"✅ 패키지 상품 유형 확인: {package_type.id}")
except ShoppingProductType.DoesNotExist:
    print("❌ 패키지 상품 유형이 존재하지 않습니다. 먼저 마이그레이션을 실행해주세요.")
    sys.exit(1)

# 기존 패키지 삭제 (중복 방지)
deleted_count, _ = ShoppingProduct.objects.filter(
    name="GLI 글렌메리 골프 & 힐링 프리미엄 투어"
).delete()
print(f"🗑️  기존 패키지 삭제: {deleted_count}개")
...
print(f"\n🌐 스테이징 환경에서 확인:")
print(f"   Admin: https://stg-admin.gli.biz/shopping/management")
print(f"   User: https://stg.gli.biz/shopping/package/{product.id}")
