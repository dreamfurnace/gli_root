...
test.describe("Investment Portfolio", () => {
	...
	test("사용자가 투자 포트폴리오 대시보드를 볼 수 있다", async ({ page }) => {
		...
		// 총 투자금액 카드 확인
		await expect(
			page.locator(".summary-card.total-investment .amount")
		).toContainText("25,000 GLIB");

		// 현재 가치 카드 확인
		await expect(
			page.locator(".summary-card.current-value .amount")
		).toContainText("27,350 GLIB");

		// 손익 카드 확인
		await expect(
			page.locator(".summary-card.profit-loss .amount")
		).toContainText("+2,350 GLIB");
		await expect(page.locator(".summary-card.profit-loss .amount")).toHaveClass(
			/profit/
		);
		await expect(
			page.locator(".summary-card.profit-loss .change-indicator")
		).toContainText("+9.40%");

		// 진행 중인 투자 카드 확인
		await expect(
			page.locator(".summary-card.active-investments .amount")
		).toContainText("3건");
	});
	...
	test("카테고리별 분산 분석이 표시된다", async ({ page }) => {
		...
		await expect(
			resortBreakdown.locator(".breakdown-percentage")
		).toContainText("60%"); // 15000/25000
		await expect(resortBreakdown.locator(".value"))
			.first()
			.toContainText("15,000 GLIB");
	...
	test("위험도별 분산 분석이 표시된다", async ({ page }) => {
		...
		await expect(lowRisk.locator(".amount")).toContainText("8,000 GLIB");
	...
	test("최근 투자 내역이 표시된다", async ({ page }) => {
		...
		await expect(firstInvestment.locator(".asset-name")).toContainText(
			"제주 프리미엄 리조트"
		);
		await expect(firstInvestment.locator(".investment-date")).toContainText(
			"2024년 1월 15일"
		);
		await expect(firstInvestment.locator(".value").first()).toContainText(
			"10,000 GLIB"
		);
		await expect(firstInvestment.locator(".value").nth(1)).toContainText(
			"10,800 GLIB"
		);

		// 수익률 확인
		await expect(firstInvestment.locator(".profit-loss")).toContainText(
			"+800 GLIB"
		);
		await expect(firstInvestment.locator(".profit-loss")).toHaveClass(/profit/);
		await expect(firstInvestment.locator(".profit-percentage")).toContainText(
			"(+8.00%)"
		);

		// 상태 배지 확인
		await expect(firstInvestment.locator(".status-badge.active")).toContainText(
			"투자 중"
		);
	});
	...
	test("투자 상태별 스타일이 올바르게 적용된다", async ({ page }) => {
		...
		await expect(
			activeInvestment.locator(".status-badge.active")
		).toBeVisible();

		// 완료된 투자 상태 확인
		const completedInvestment = page.locator(".investment-item").nth(2);
		await expect(
			completedInvestment.locator(".status-badge.completed")
		).toContainText("완료");
	...
	test("투자 내역이 없는 경우 빈 상태가 표시된다", async ({ page }) => {
		...
		await expect(
			page.locator('h3:has-text("투자 내역이 없습니다")')
		).toBeVisible();
		await expect(
			page.locator('p:has-text("RWA 자산에 투자하여")')
		).toBeVisible();

		// 투자하러 가기 버튼 확인
		await expect(page.locator(".start-investing-btn")).toContainText(
			"투자하러 가기"
		);
		...
		await page.waitForTimeout(100);
		expect(logs.some((log) => log.includes("Go to investments page"))).toBe(
			true
		);
	});
	...
	test("손실이 있는 투자의 경우 적절한 스타일이 적용된다", async ({ page }) => {
		...
		// 개별 투자 손실 확인
		await expect(page.locator(".investment-item .profit-loss")).toContainText(
			"-750 GLIB"
		);
		await expect(page.locator(".investment-item .profit-loss")).toHaveClass(
			/loss/
		);
		await expect(
			page.locator(".investment-item .profit-percentage")
		).toContainText("(-7.50%)");
	});
	...
	test("로딩 상태가 표시된다", async ({ page }) => {
		...
		await expect(
			page.locator('text="포트폴리오 정보를 불러오는 중"')
		).toBeVisible();
	...
	test("에러 상태가 처리된다", async ({ page }) => {
		...
		await expect(
			page.locator('h3:has-text("데이터를 불러올 수 없습니다")')
		).toBeVisible();
	...
	test("숫자 포맷팅이 올바르게 작동한다", async ({ page }) => {
		...
		// 큰 숫자의 쉼표 구분 확인
		await expect(page.locator(".summary-card .amount").first()).toContainText(
			"25,000"
		);

		// 소수점 표시 확인
		await expect(page.locator(".change-indicator").first()).toContainText(
			"9.40%"
		);

		// 개별 투자 금액 포맷 확인
		await expect(page.locator(".investment-item .value").first()).toContainText(
			"10,000 GLIB"
		);
	});
	...
	test("백분율 계산이 정확하다", async ({ page }) => {
		...
		await expect(page.locator(".breakdown-percentage").first()).toContainText(
			"60%"
		);
