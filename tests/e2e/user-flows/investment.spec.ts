...
test.describe("RWA Investment Flow", () => {
						...
						results: [
							{
								...
								name: "제주 프리미엄 리조트",
								short_description:
									"제주도 해안가에 위치한 프리미엄 리조트 투자",
								expected_apy: 8.5,
								risk_level: "medium",
								risk_level_display: "보통",
								min_investment_glib: 1000,
								max_investment_glib: 50000,
	...
	test("사용자가 자산을 필터링할 수 있다", async ({ page }) => {
		...
		await expect(page.locator(".quick-filter.active")).toContainText(
			"추천 자산"
		);
	});
	...
	test("사용자가 투자 모달을 열고 투자할 수 있다", async ({ page }) => {
		// 투자 API 모킹
		await page.route(
			"**/api/v1/rwa/assets/test-asset-1/invest/",
			async (route) => {
				await route.fulfill({
					status: 201,
					body: JSON.stringify({
	...
	test("빈 결과 상태가 표시된다", async ({ page }) => {
		...
		await expect(
			page.locator('h3:has-text("검색 결과가 없습니다")')
		).toBeVisible();
